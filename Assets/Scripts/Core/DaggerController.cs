...
public class DaggerController : MonoBehaviour
{
    ...
    List<GameObject> dagger_list = new List<GameObject>();
    GameObject blink_dagger = null;
    ...
    void Update()
    {
        ...
        /*GameObject Target = findBlinkTarget();
        if(Target != null)
        {
            foreach(GameObject d in dagger_list)
            {
                if (d != Target)
                    d.GetComponent<Animator>().SetBool("Nearest", false);
            }
            if(heavy_dagger != null && heavy_dagger != Target)
                heavy_dagger.GetComponent<Animator>().SetBool("Nearest", false);
            Target.GetComponent<Animator>().SetBool("Nearest", true);
            if (Input.GetMouseButtonDown(1) && blink_cd <= 0)
            {
                blink(Target.GetComponent<Dagger>());
            }
        }
        else
        {
        ...
        if (blink_dagger != null && !Input.GetMouseButton(1) && blink_cd <= 0)
        {
            blink(blink_dagger.GetComponent<Dagger>());
        }
    ...
    void ControlDefault()
    {
        ...
        {
            if (dagger_list.Count == max_daggers)
                collectDagger(0);
            dagger_list.Add(throwDagger(dagger_prefabs[0]));
        }
    }
    void ControlBlink()
    {
        if (Input.GetMouseButtonDown(1) && cooldown <= 0 && charge <= 0)
        {
            if (blink_dagger != null)
                collectDagger(blink_dagger);
            blink_dagger = throwDagger(dagger_prefabs[2]);
            blink_dagger.GetComponent<Animator>().SetBool("Nearest", true);
        }
    }
    ...
    void ControlHeavy()
    {
        if (heavy_dagger == null)
        {
            ...
            {
                heavy_dagger = throwDagger(dagger_prefabs[1], dgr_charge_curve.Evaluate(charge));
                resetCharge();
            }
        }
        else
    ...
    GameObject throwDagger(GameObject prefab, float speed_mult = 1)
    {
        GameObject dgr = Instantiate(prefab, transform.position, new Quaternion());
        Vector2 dir = Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, 0)) - transform.position;
        dir.Normalize();
        dgr.GetComponent<Rigidbody2D>().velocity = dir * dgr_throw_coeff * speed_mult;
        if (dir.x < 0)
